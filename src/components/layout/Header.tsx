...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
      {/* Announcement bar */}
      <div className="bg-primary text-primary-foreground text-[10px] sm:text-xs text-center py-2 px-4 font-medium">
        <span className="inline-flex items-center gap-1.5">
          <Zap className="w-3 h-3" fill="currentColor" />
          <span className="hidden sm:inline">
            Free shipping in Ulaanbaatar on orders over 500,000₮
          </span>
          <span className="sm:hidden">Free shipping in UB on orders over 500,000₮</span>
        </span>
      </div>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="container mx-auto px-3 sm:px-4 h-14 sm:h-16 flex items-center justify-between gap-3 sm:gap-4">
          <Link to="/" className="flex items-center gap-2 group shrink-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-primary grid place-items-center transition-transform group-hover:scale-105">
              <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-primary-foreground" fill="currentColor" />
            </div>
            <div className="font-display font-bold text-lg sm:text-xl tracking-tight leading-none text-gray-900">
              MINI <span className="text-primary hidden xs:inline">MOTORS</span>
              <span className="text-primary xs:hidden">M</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            >
              ...
              {count > 0 && (
                <span className="absolute -top-1 -right-1 bg-primary text-primary-foreground text-[10px] font-bold w-4 h-4 sm:w-5 sm:h-5 rounded-full grid place-items-center">
                  {count > 99 ? "99+" : count}
                </span>
              )}
            </Link>
